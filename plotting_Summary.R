...
###############################################
# Figure 1: Abundance, viral titer
...
p2c_viral_titer <- ggplot(guinardia_data, aes(hours, Viral_titer)) + 
    ...
    scale_x_continuous(breaks=c(seq(0,92,10)))+
    theme_minimal() + 
    labs(title = "B  Extracellular infectious RNA viruses",x="Time post infection (hours)",y="Viral titer [1/mL]")

grid.newpage()
grid.draw(rbind(ggplotGrob(p2a_abundances), ggplotGrob(p2c_viral_titer),size = "last"))
...
#  facet_grid(Culture~.)+ theme_minimal()
